...
from .transformations import threshold, rgb_to_monochromatic
...
def compute_angles(imgs_grid):
    preprocess = lambda img: threshold(apply_gaussian_filter(img, 2), 0.8)
...
def compute_translations(imgs_grid):
    preprocess = lambda img: threshold(apply_gaussian_filter(img, 3), 0.7)
    used_imgs_grid = list(map(lambda row: list(map(preprocess, row)), imgs_grid))
...
def preprocess(img):
    return threshold(apply_gaussian_filter(rgb_to_monochromatic(img), 3), 0.7)

def stitch_images(imgs_grid):
    gray_imgs_grid = list(map(
        lambda row: list(map(rgb_to_monochromatic, row)),
        imgs_grid
    ))
